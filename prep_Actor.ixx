...
)
{
	...
	auto pool_110 = *reinterpret_cast<byte8 ***>(appBaseAddr + 0xC90E28);
	if
	(
		!pool_110 ||
		!pool_110[8]
	...
	auto & enemyVectorData = *reinterpret_cast<EnemyVectorData *>(pool_110[8]);
...
)
{
	...
	[&]()
	{
		...
		auto pool_220 = *reinterpret_cast<byte8 ***>(appBaseAddr + 0xC90E28);
		if
		(
			!pool_220 ||
			!pool_220[3]
		...
		auto & mainActorData = *reinterpret_cast<PlayerActorData *>(pool_220[3]);
...
)
{
	...
	auto name_4480 = *reinterpret_cast<byte8 **>(appBaseAddr + 0xC90E30);
	if (!name_4480)
	{
		return 0;
	}
	auto & queuedMissionActorData = *reinterpret_cast<QueuedMissionActorData *>(name_4480 + 0xC0 );
	auto & activeMissionActorData = *reinterpret_cast<ActiveMissionActorData *>(name_4480 + 0x16C);
...
)
{
	...
	switch (characterData.character)
	{
		...
		case CHARACTER::BOSS_LADY:
		{
			[&]()
			{
				auto pool_4685 = *reinterpret_cast<byte8 ***>(appBaseAddr + 0xC90E28);
				if
				(
					!pool_4685 ||
					!pool_4685[3]
				...
				auto mainActorBaseAddr = pool_4685[3];
		...
		case CHARACTER::BOSS_VERGIL:
		{
			[&]()
			{
				auto pool_4702 = *reinterpret_cast<byte8 ***>(appBaseAddr + 0xC90E28);
				if
				(
					!pool_4702 ||
					!pool_4702[3]
				...
				auto mainActorBaseAddr = pool_4702[3];
				auto & mainActorData = *reinterpret_cast<PlayerActorData *>(mainActorBaseAddr);
...
void LinearMeleeWeaponSwitchController(T & actorData)
{
	...
	[&]()
	{
		auto name_5230 = *reinterpret_cast<byte8 **>(appBaseAddr + 0xC90E28);
		if (!name_5230)
		{
			return;
		}
		name_5230 -= 0x180;
		auto hudTop    = *reinterpret_cast<byte8 **>(name_5230 + 0x1B070);
		auto hudBottom = *reinterpret_cast<byte8 **>(name_5230 + 0x1B078);
...
void LinearRangedWeaponSwitchController(T & actorData)
{
	...
	[&]()
	{
		auto name_5342 = *reinterpret_cast<byte8 **>(appBaseAddr + 0xC90E28);
		if (!name_5342)
		{
			return;
		}
		name_5342 -= 0x180;
		auto hudTop    = *reinterpret_cast<byte8 **>(name_5342 + 0x1B070);
		auto hudBottom = *reinterpret_cast<byte8 **>(name_5342 + 0x1B078);
...
)
{
	...
	uint8 * dashCount =
	(actorData.styleLevel == 0) ? dashCount0 :
	(actorData.styleLevel == 1) ? dashCount1 :
...
void UpdateActorSpeed(byte8 * baseAddr)
{
	...
	auto pool_10840 = *reinterpret_cast<byte8 ***>(appBaseAddr + 0xC90E28);
	if
	(
		!pool_10840 ||
		!pool_10840[3]
	...
	auto & mainActorData = *reinterpret_cast<PlayerActorData *>(pool_10840[3]);
...
)
{
	...
	[&]()
	{
		auto pool_11260 = *reinterpret_cast<byte8 ***>(appBaseAddr + 0xC90E28);
		if
		(
			!pool_11260 ||
			!pool_11260[3]
		...
		auto actorBaseAddr = pool_11260[3];
		auto & actorData = *reinterpret_cast<PlayerActorData *>(actorBaseAddr);
	...
	if (!match)
	{
		...
		auto pool_11300 = *reinterpret_cast<byte8 ***>(appBaseAddr + 0xC90E28);
		if
		(
			!pool_11300 ||
			!pool_11300[3]
		...
		auto & actorData = *reinterpret_cast<PlayerActorData *>(pool_11300[3]);
...
bool DotShadowCheck(byte8 * dest)
{
	...
	switch (activeConfig.dotShadow)
	{
		...
		case DOT_SHADOW::DISABLE_PLAYER_ACTORS_ONLY:
		{
			...
			// Main Actor
			{
				auto pool_11898 = *reinterpret_cast<byte8 ***>(appBaseAddr + 0xC90E28);
				if
				(
					!pool_11898 ||
					!pool_11898[3]
				...
				auto actorBaseAddr = pool_11898[3];
				auto & actorData = *reinterpret_cast<PlayerActorData *>(actorBaseAddr);
...
export void SceneGame()
{
	...
	auto pool_19687 = *reinterpret_cast<byte8 ***>(appBaseAddr + 0xC90E10);
	if
	(
		!pool_19687 ||
		!pool_19687[8]
	...
	auto & eventData = *reinterpret_cast<EventData *>(pool_19687[8]);

	auto pool_19698 = *reinterpret_cast<byte8 ***>(appBaseAddr + 0xC90E10);
	if
	(
		!pool_19698 ||
		!pool_19698[12]
	...
	auto & nextEventData = *reinterpret_cast<NextEventData *>(pool_19698[12]);

	auto pool_19709 = *reinterpret_cast<byte8 ***>(appBaseAddr + 0xC90E30);
	if
	(
		!pool_19709 ||
		!pool_19709[1]
	...
	auto eventFlags = reinterpret_cast<byte32 *>(pool_19709[1]);
